...
def test_collect_all_responses_stops_on_empty_page(
    ...
    responses, interrupted = collect_all_responses(
        "https://dic.nicovideo.jp/b/a/12345/"
    )
    assert responses == [{"res_no": 1}, {"res_no": 2}]
...
@patch("orchestrator.parse_responses")
@patch("orchestrator.fetch_page")
def test_collect_all_responses_marks_later_page_interruption_and_keeps_partial_data(
    mock_fetch, mock_parse, mock_sleep
):
    mock_fetch.side_effect = [MagicMock(), RuntimeError("network unavailable")]
...
def test_run_scrape_saves_partial_result_after_later_page_interruption():
    article_url = "https://dic.nicovideo.jp/a/12345"
    ...
    conn.close.assert_called_once_with()
    mock_print.assert_any_call("Later-page fetch interrupted; saving partial result")
    mock_print.assert_any_call("Saved to SQLite")
