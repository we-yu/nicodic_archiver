"""
Thin archive read seam: SQLite fetch only, no CLI formatting.

Read-facing code (CLI today; Web/API later) can reuse these functions
without importing presentation helpers.
"""
...
def fetch_article_archive(
    ...
    *,
    ...
    """Load one article and its responses from the archive DB."""
...
def fetch_article_summaries(
    *,
    ...
    """Return saved article rows with response counts for listing / export-all."""
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
