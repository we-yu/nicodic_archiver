...
def _run_batch_from_target_list_path(target_list_path: str) -> tuple[int, int, str]:
    """
    Run one batch pass (one巡回) over all targets in a list file.
    Reuses existing single-article flow (`run_scrape`) and existing run logging.

    Returns:
        (failed_targets, total_targets, final_status)
    """
    ...
    return failed_targets, total_targets, final_status
...
def main():
    ...
    if sys.argv[1] == "batch":
        ...
        target_list_path = sys.argv[2]
        failed_targets, _, _ = _run_batch_from_target_list_path(target_list_path)
        if failed_targets:
            sys.exit(1)
        return
    ...
    if sys.argv[1] == "periodic":
        ...
        try:
            while True:
                run_no += 1
                _run_batch_from_target_list_path(target_list_path)

                if max_runs is not None and run_no >= max_runs:
                    return

                time.sleep(interval_seconds)
        except KeyboardInterrupt:
            print("Periodic execution interrupted (Ctrl+C).")
            return
