...
## Known-good smoke (KGS) (TASK033B)

KGS is a manual, opt-in, non-gating live smoke helper.

Rules:
- KGS live actions require `--isolated` and use isolated smoke state only.
- KGS helper messages are stdout-only (no new persistent KGS log layer).
- KGS is not CI/PR gating and not a scheduler platform.

### Configure a known-good target

You can set the KGS target in one of these ways (priority order):
- flag: `--known-good-url <canonical_url>`
- file: `--kgs-file <path>` (or `tools/kgs.txt` if present)
- env: `NICODIC_KGS_URL`

Template file:
- `tools/kgs.example.txt`

### One-shot KGS smoke (isolated)

Batch semantics:

```sh
python main.py verify kgs smoke --isolated --mode batch \
  --known-good-url https://dic.nicovideo.jp/a/12345
```

Periodic-once semantics:

```sh
python main.py verify kgs smoke --isolated --mode periodic \
  --known-good-url https://dic.nicovideo.jp/a/12345
```

### Bounded incremental follow-up smoke (isolated)

This follow-up intentionally deletes only the last N response rows in the
isolated DB, then re-fetches the same KGS target to verify bounded resume.
